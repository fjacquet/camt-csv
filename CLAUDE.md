...
Directory processing is **not** a parser concern: `batch.BatchProcessor` (`internal/batch/processor.go`) composes a `FullParser` with an `OutputFormatter`, groups every file's transactions by account, and writes one date-sorted CSV per account plus a single run report (`batch.AccountOutputPathFor` names the CSVs, e.g. `releves.csv` → `releves_54293249.csv`; `batch.ManifestPathFor` names the report, `releves.manifest.json`). Accounts come from the statement itself where the format carries one — CAMT.053's `<Stmt><Acct>`, landing on `Transaction.IBAN` — and from the file name (`common.AccountKeyFromFilename`) only for formats that do not (PDF, Revolut, Selma). Content wins over the name: a renamed or hand-edited file must not send rows to the wrong account. Both normalize to the same key, the trailing digit run (`CH1700767000K54293249` → `54293249`, `common.AccountKeyFromIBAN`), so one account never splits across two CSVs because of how a file was named. Attribution is per transaction, so one document covering several statements splits correctly. Rows with neither source go to `releves_unknown.csv`, and a merge of several such files is warned about. `cmd/convert.convertDirectory` is the single CLI entry point to it.
...
CLI usage: `camt-csv convert -i <input> -o <output> --format standard|icompta|jumpsoft`. A directory input produces one date-sorted CSV per account (`-o releves.csv` → `releves_54293249.csv`, `releves_unknown.csv`) — never the unsuffixed name, and never one CSV per input file. The `--date-format` flag is deprecated and has no effect — output dates are always `DD.MM.YYYY` (`models.DateFormatCSV`). Directory input takes `--recursive` to descend into subdirectories. New formatters: implement `OutputFormatter` interface, register via `registry.Register("name", formatter)`.
...
- `IBAN` is the account the row belongs to (the reader's own), set by the CAMT parser from `<Stmt><Acct>` and used to group a batch by account; `PartyIBAN` is the counterparty's. No other parser populates it yet.
- Never populated by any parser: `AmountExclTax`, `TaxRate`, `EntryReference`, `BankTxCode`
