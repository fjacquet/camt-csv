...
Directory processing is **not** a parser concern: `batch.BatchProcessor` (`internal/batch/processor.go`) composes a `FullParser` with an `OutputFormatter`, groups every file's transactions by account, and writes one date-sorted CSV per account plus a single run report (`batch.AccountOutputPathFor` names the CSVs, e.g. `releves.csv` → `releves_54293249.csv`; `batch.ManifestPathFor` names the report, `releves.manifest.json`). Accounts come from file names (`common.AccountKeyFromFilename`), never from statement content; files carrying no account number go to `releves_unknown.csv`. `cmd/convert.convertDirectory` is the single CLI entry point to it.
