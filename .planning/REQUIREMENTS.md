...
- [ ] **FMT-01**: `--format jumpsoft` option available on all parser commands
- [ ] **FMT-02**: JumpsoftFormatter outputs comma-delimited CSV with header row
- [ ] **FMT-03**: Date field uses `YYYY-MM-DD` format (configurable via `--date-format` flag)
- [ ] **FMT-04**: Amount field is signed (negative for debits, positive for credits)
- [ ] **FMT-05**: Category field populated from categorizer output

### Integration

- [ ] **INT-01**: JumpsoftFormatter registered in FormatterRegistry as `"jumpsoft"`
- [ ] **INT-02**: `--format jumpsoft` works in single-file mode across all 6 parsers (camt, pdf, revolut, selma, debit, revolut-investment)
- [ ] **INT-03**: `--format jumpsoft` works in folder mode across all parsers
- [ ] **INT-04**: `--format jumpsoft` documented in CLI help output
...
|-------------|-------|--------|
| FMT-01 | Phase 14 | Pending |
| FMT-02 | Phase 14 | Pending |
| FMT-03 | Phase 14 | Pending |
| FMT-04 | Phase 14 | Pending |
| FMT-05 | Phase 14 | Pending |
