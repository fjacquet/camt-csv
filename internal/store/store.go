...
func (s *CategoryStore) LoadCategories() ([]models.CategoryConfig, error) {
	...
	// Attempt to unmarshal with detailed struct
	var categories []models.CategoryConfig
	err = yaml.Unmarshal(data, &categories)
	if err != nil {
		// If standard unmarshal fails, try parsing manually for backward compatibility
		return s.parseExistingCategoriesFile(data)
	}
	
	log.Infof("Loaded %d categories from %s", len(categories), filePath)
...
func (s *CategoryStore) parseExistingCategoriesFile(data []byte) ([]models.CategoryConfig, error) {
	...
	log.Infof("Parsed %d categories from custom format", len(categories))
	return categories, nil
}
...
func (s *CategoryStore) LoadCreditorMappings() (map[string]string, error) {
	...
	log.Infof("Loaded %d creditor mappings from %s", len(mappings), filePath)
	return mappings, nil
}
...
func (s *CategoryStore) LoadDebitorMappings() (map[string]string, error) {
	...
	log.Infof("Loaded %d debitor mappings from %s", len(mappings), filePath)
	return mappings, nil
}
...
func (s *CategoryStore) SaveCreditorMappings(mappings map[string]string) error {
	...
	// Use absolute path if provided, otherwise save to current directory
	var filePath string
	if filepath.IsAbs(filename) {
		filePath = filename
	} else {
	...
	log.Infof("Saved %d creditor mappings to %s", len(mappings), filePath)
	return nil
}
...
func (s *CategoryStore) SaveDebitorMappings(mappings map[string]string) error {
	...
	log.Infof("Saved %d debitor mappings to %s", len(mappings), filePath)
	return nil
}
